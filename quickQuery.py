...
CACHE_DIR = os.path.join(XDG_CACHE_DIR, 'quickQuery')
...
def get_result(url):
    r = requests.get(url, headers={'User-Agent': random.choice(USER_AGENTS)}, proxies=get_proxies()).text
    return r
    #pr_debug('get_result', url)
    #req = urllib2.Request(url, headers=headers)
    #res = urllib2.urlopen(req).read()

    #for debug
    #file_result = open(os.getcwd() + 'query_result' + '.html', 'w+')
...
def command_line_runner():
    ...
    #使能cache
    if not os.getenv('QUICKQUERY_DISABLE_CACHE'):
        enable_cache()

    if sys.version < '3':
        print(quickQuery(args).encode('utf-8', 'ignore'))
    else:
        print(quickQuery(args))
